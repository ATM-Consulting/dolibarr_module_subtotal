		...
		if($action === 'builddoc' || $action === 'addline' || $action === 'confirm_valid' || $action === 'confirm_paiement') {
			...
			if($line->product_type == 9 && $line->special_code == $this->module_number) {
				...
				if($line->qty>90) {
					...
					$substitutionarray['line_price_ht'] = price($TInfo[0],0,'',1,2,1);
					$substitutionarray['line_price_vat'] = price($TInfo[1],0,'',1,2,1);
					$substitutionarray['line_price_ttc'] = price($TInfo[2],0,'',1,2,1);
				} else {
...
//		var_dump($bMargin);
		...
		if (!$hidePriceOnSubtotalLines) {
			$total_to_print = price($line->total,0,'',1,2,1);
...
//					$line->total_ttc
				...
				else
				{
					...
					$TInfo = $this->getTotalLineFromObject($object, $line, '', 1);
					$TTotal_tva = $TInfo[3];
					$total_to_print = price($TInfo[0],0,'',1,2,1);
...
//		$pdf->SetTextColor('text', 0, 0, 0);
	...
	function pdf_getlinetotalexcltax($parameters=array(), &$object, &$action='') {
		...
		if (GETPOST('hideInnerLines', 'int') && !empty($conf->global->SUBTOTAL_REPLACE_WITH_VAT_IF_HIDE_INNERLINES)){
		    $this->resprints = price($object->lines[$i]->total_ht,0,'',1,2,1);
		}
		...
		if($this->isModSubtotalLine($parameters,$object) ) {
            ...
            if(TSubtotal::isSubtotal($line)) {
                ...
                if(! empty($parentTitle->array_options['options_show_total_ht'])) {
                    ...
                    $this->resprints = price($TTotal['total_unit_subprice'],0,'',1,2,1);
                }
            }
