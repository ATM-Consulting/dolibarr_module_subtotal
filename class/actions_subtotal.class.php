		 	...
		 	<script type="text/javascript">
				$(document).ready(function() {
					...
					function promptSubTotal(action, titleDialog, label, url_to, url_ajax, params, use_textarea, show_free_text, show_under_title) {
						...
						<?php
						$editorTool = !getDolGlobalString('FCKEDITOR_EDITORNAME')?'ckeditor':getDolGlobalString('FCKEDITOR_EDITORNAME');
						$editorConf = !getDolGlobalString('FCKEDITOR_ENABLE_DETAILS')?false:getDolGlobalString('FCKEDITOR_ENABLE_DETAILS');
		...
		else if (in_array('invoicecard',$contexts) || in_array('invoicesuppliercard',$contexts) || in_array('propalcard',$contexts) || in_array('supplier_proposalcard',$contexts) || in_array('ordercard',$contexts) || in_array('ordersuppliercard',$contexts) || in_array('invoicereccard',$contexts))
        {
			...
			<td align="center" class="nowrap linecoldelete">
				...
				if ($action != 'editline' && $action != 'selectlines') {
						if ($object->statut == 0  && $createRight && getDolGlobalString('SUBTOTAL_ALLOW_REMOVE_BLOCK'))
						{
                            if(empty($line->fk_prev_id)) $line->fk_prev_id = null;
			...
			<?php
			if ($object->statut == 0  && $createRight && getDolGlobalString('SUBTOTAL_MANAGE_COMPRIS_NONCOMPRIS') && TSubtotal::isTitle($line) && $action != 'editline' && $action != 'selectlines')
			{
			...
			<?php } ?>
				...
				if (getDolGlobalString('MASSACTION_CARD_ENABLE_SELECTLINES') && $object->status == $object::STATUS_DRAFT && $usercandelete && in_array($object->element,$Telement)|| $action == 'selectlines' ) { // dolibarr 8
