	...
	function printObjectLine($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($line->special_code!=$this->module_number || $line->product_type!=9) {
			...
			if (!empty($conf->global->SUBTOTAL_USE_NEW_FORMAT)) {
				...
				elseif ($line->qty==98) print 'background:#ddddff;';
				elseif ($line->qty<=97 && $line->qty>=91) print 'background:#eeeeff;';
				elseif ($line->qty==1) print 'background:#adadcf;';
				elseif ($line->qty==2) print 'background:#ddddff;';
				elseif ($line->qty==50) print '';
				...
				elseif ($line->qty==98) print 'background:#ddddff;';
				elseif ($line->qty==2) print 'background:#eeeeff; ';
				elseif ($line->qty==50) print '';
			...
			<td align="center" class="nowrap linecoldelete">
				...
				if ($action != 'editline' && $action != 'selectlines') {
					if ($object->statut == 0  && $createRight && !empty($conf->global->SUBTOTAL_ALLOW_REMOVE_BLOCK)) {
						...
						if (TSubtotal::isTitle($line) && (isset($line->fk_prev_id)) ) {
		...
		if (in_array('ordercard', $contexts)
			...
			if (TSubtotal::isModSubtotalLine($line)) {
				...
				if (!empty($conf->global->SUBTOTAL_USE_NEW_FORMAT)) {
					...
					elseif ($line->qty==98) $object->tpl['sub-tr-style'].= 'background:#ddddff;';
					elseif ($line->qty<=97 && $line->qty>=91) $object->tpl['sub-tr-style'].= 'background:#eeeeff;';
					elseif ($line->qty==1) $object->tpl['sub-tr-style'].= 'background:#adadcf;';
					elseif ($line->qty==2) $object->tpl['sub-tr-style'].= 'background:#ddddff;';
					elseif ($line->qty==50) $object->tpl['sub-tr-style'].= '';
					...
					elseif ($line->qty==98) $object->tpl['sub-tr-style'].= 'background:#ddddff;';
					elseif ($line->qty==2) $object->tpl['sub-tr-style'].= 'background:#eeeeff; ';
					elseif ($line->qty==50) $object->tpl['sub-tr-style'].= '';
