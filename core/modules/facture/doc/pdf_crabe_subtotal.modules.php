...
class pdf_crabe_subtotal extends ModelePDFFactures
{
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$this->pageHead($pdf, $object, 1, $outputlangs);
				...
				for ($i = 0; $i < $nblignes; $i++) {
					...
					if (isset($imglinesize['width']) && isset($imglinesize['height']) && ($curY + $imglinesize['height']) > ($this->page_hauteur-($heightforfooter+$heightforfreetext+$heightforinfotot))) {	// If photo too high, we moved completely on new page
						$pdf->AddPage('', '', true);
						if (! empty($tplidx)) $pdf->useTemplate($tplidx);
						if (!getDolGlobalString('MAIN_PDF_DONOTREPEAT_HEAD')) $this->pageHead($pdf, $object, 0, $outputlangs);
					...
					while ($pagenb < $pageposafter) {
						$pdf->setPage($pagenb);
						if ($pagenb == 1) {
							$this->tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforfooter, 0, $outputlangs, 0, 1);
						} else {
							$this->tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforfooter, 0, $outputlangs, 1, 1);
						}
				...
				// Show square
				if ($pagenb == 1) {
					$this->tableau($pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 0, 0);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				} else {
					$this->tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter, 0, $outputlangs, 1, 0);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}

				// Affiche zone infos
				$posy=$this->tableauInfo($pdf, $object, $bottomlasttab, $outputlangs);

				if (!getDolGlobalString('SUBTOTAL_HIDE_DOCUMENT_TOTAL')) {
					// Affiche zone totaux
					$posy=$this->tableauTot($pdf, $object, $deja_regle, $bottomlasttab, $outputlangs);
				}

				// Affiche zone versements
				if ($deja_regle || $amount_credit_notes_included || $amount_deposits_included) {
					$posy=$this->tableauVersements($pdf, $object, $posy, $outputlangs);
				}
	 ...
	 */
	private function tableauVersements(&$pdf, $object, $posy, $outputlangs)
	{
		global $conf, $db;
	 ...
	 *   @return	void
	 */
	private function tableauInfo(&$pdf, $object, $posy, $outputlangs)
	{
		global $conf;
	 ...
	 */
	private function tableauTot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		global $conf,$mysoc;
	 ...
	 *   @return	void
	 */
	private function tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
		global $conf;
	 ...
	 *  @return	void
	 */
	private function pageHead(&$pdf, $object, $showaddress, $outputlangs)
	{
		global $conf,$langs;
	 ...
	 */
	private function pageFoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		$showdetails=0;
